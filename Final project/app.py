...
app = Flask(__name__)
app.secret_key = os.getenv("SECRET_KEY", "college-event-portal-secret")
...
def get_created_events_with_competitions() -> list[dict]:
    ...
    for event in events:
        ...
        historic_data = build_historic_event_snapshot(event.get("title") or "")
        event["historic_competition_count"] = len(historic_data["competitions"])
        event["historic_result_count"] = len(historic_data["results"])
        event["announcement_count"] = fetch_one(
            "SELECT COUNT(*) AS total FROM event_result_announcements WHERE portal_event_id = ?",
            (event["id"],),
        )["total"]
        event["result_count"] = event["announcement_count"] + event["historic_result_count"]
        event["display_competition_count"] = max(
            len(event["competitions"]),
            to_int(event.get("competition_count")) or 0,
            event["historic_competition_count"],
        )
        event["display_total_prize_money"] = max(
            to_int(event.get("total_prize_money")) or 0,
            sum(
                (to_int(item.get("first_prize")) or 0)
                + (to_int(item.get("second_prize")) or 0)
                + (to_int(item.get("third_prize")) or 0)
                for item in historic_data["competitions"]
            ),
        )
    return events


def build_historic_event_snapshot(event_name: str) -> dict:
    # Reuses old event_records rows so finished events like Samyuti appear in the live event detail page.
    normalized_name = (event_name or "").strip()
    ...
    rows = [
        ...
        for row in fetch_all(
            ...
            ORDER BY event_date DESC, competition_name ASC, id DESC
            """,
            (normalized_name,),
        )
    ]
    ...
    for row in rows:
        competition_name = (row.get("competition_name") or row.get("event_name") or "").strip()
        if not competition_name:
            continue

        competition = competition_map.setdefault(
            competition_name,
            {
                "competition_name": competition_name,
                "venue": row.get("venue") or "",
                "max_team_members": max(1, to_int(row.get("team_size")) or 1),
                "first_prize": 0,
                "second_prize": 0,
                "third_prize": 0,
            },
        )
        if not competition.get("venue") and row.get("venue"):
            competition["venue"] = row["venue"]
        competition["max_team_members"] = max(
            to_int(competition.get("max_team_members")) or 1,
            to_int(row.get("team_size")) or 1,
        )

        position = str(row.get("position") or "").strip()
        prize_money = to_int(row.get("prize_money")) or 0
        if position == "1":
            competition["first_prize"] = max(to_int(competition.get("first_prize")) or 0, prize_money)
        elif position == "2":
            competition["second_prize"] = max(to_int(competition.get("second_prize")) or 0, prize_money)
        elif position == "3":
            competition["third_prize"] = max(to_int(competition.get("third_prize")) or 0, prize_money)

        if position or row.get("result") or row.get("student_name"):
            result_rows.append(
                {
                    "id": f"historic-{row.get('id')}",
                    "competition_name": competition_name,
                    "position": position,
                    "result_label": row.get("result") or (f"Position {position}" if position else "-"),
                    "prize_money": prize_money,
                    "participant_name": row.get("student_name") or "-",
                    "registration_number": row.get("student_id") or "",
                    "team_name": row.get("team_id") or "",
                    "school_name": row.get("school_name") or "",
                    "department": row.get("department") or "",
                    "study_year": row.get("study_year") or "",
                    "source": "historic",
    ...
    competitions = sorted(competition_map.values(), key=lambda item: str(item.get("competition_name") or "").lower())
    results = sorted(
        result_rows,
        key=lambda item: (
            str(item.get("competition_name") or "").lower(),
            to_int(item.get("position")) or 999,
            str(item.get("participant_name") or "").lower(),
        ),
    )
    return {"competitions": competitions, "results": results}


def merge_event_competitions(live_competitions: list[dict], historic_competitions: list[dict]) -> list[dict]:
    # Combines configured live competitions with inferred historic competition data.
    merged: dict[str, dict] = {}
    for competition in historic_competitions + live_competitions:
        competition_name = (competition.get("competition_name") or "").strip()
        if not competition_name:
            continue
        ...
        for key in ["id", "venue", "created_at"]:
            if competition.get(key) and not existing.get(key):
                existing[key] = competition.get(key)
        existing["max_team_members"] = max(
            to_int(existing.get("max_team_members")) or 1,
            to_int(competition.get("max_team_members")) or 1,
        )
        for prize_key in ["first_prize", "second_prize", "third_prize"]:
            existing[prize_key] = max(
                to_int(existing.get(prize_key)) or 0,
                to_int(competition.get(prize_key)) or 0,
            )
    return sorted(merged.values(), key=lambda item: str(item.get("competition_name") or "").lower())


def merge_event_results(announcements: list[dict], historic_results: list[dict]) -> list[dict]:
    # Keeps one shared result list so finished events can show historic winners beside live announcements.
    merged: list[dict] = []
    seen: set[tuple] = set()
    for source_name, rows in (("announcement", announcements), ("historic", historic_results)):
        for row in rows:
            normalized = dict(row)
            normalized["source"] = normalized.get("source") or source_name
            key = (
                normalized.get("competition_name") or "",
                str(normalized.get("position") or ""),
                normalized.get("participant_name") or "",
                normalized.get("registration_number") or "",
                str(to_int(normalized.get("prize_money")) or 0),
                normalized.get("result_label") or normalized.get("result") or "",
            )
            if key in seen:
                continue
            seen.add(key)
...
def get_event_by_id(event_id: int) -> dict | None:
    ...
    historic_data = build_historic_event_snapshot(data.get("title") or "")
    data["historic_competitions"] = historic_data["competitions"]
    data["historic_results"] = historic_data["results"]
    data["display_competitions"] = merge_event_competitions(data["competitions"], data["historic_competitions"])
    data["available_results"] = merge_event_results(data["announcements"], data["historic_results"])
    data["announcement_count"] = len(data["announcements"])
    data["available_result_count"] = len(data["available_results"])
    data["competition_count"] = max(
        to_int(data.get("competition_count")) or 0,
        len(data["display_competitions"]),
    )
    data["total_prize_money"] = max(
        to_int(data.get("total_prize_money")) or 0,
        sum(
            (to_int(item.get("first_prize")) or 0)
            + (to_int(item.get("second_prize")) or 0)
            + (to_int(item.get("third_prize")) or 0)
            for item in data["display_competitions"]
        ),
    )
    return data
...
def event_detail(event_id: int):
    ...
    announcement_competitions = sorted(
        {
            row["competition_name"]
            for row in event["available_results"]
            if row.get("competition_name")
        }
    )
    displayed_announcements = event["available_results"]
...
# Load local Supabase settings before touching the database.
